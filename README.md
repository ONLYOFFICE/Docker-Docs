...
- **AMPQ_TYPE**: Defines the message broker type. Defaults to `rabbitmq`. Possible values are `rabbitmq` or `activemq`.
- **AMQP_PROTO**: The protocol for the connection to AMQP server. If `QUEUE_TYPE=rabbitmq` - default to `amqp`. Possible values are `amqp`, `amqps`. If `QUEUE_TYPE=activemq` - Defaults to `tcp`. Possible values are `undefined`, `tcp` or `tls`.
